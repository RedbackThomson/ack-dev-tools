...
func (m *Manager) EnsureRemotes(ctx context.Context, repo *Repository) error {
	expectedOriginURL := m.urlBuilder(m.cfg.Github.Username, repo.ExpectedForkName)
	err := util.UpdateRepositoryRemotes(repo.gitRepo, originRemoteName, []string{expectedOriginURL})
	if err != nil {
		return fmt.Errorf("error updating origin URL: %v", err)
	}

	expectedUpstreamURL := m.urlBuilder(github.ACKOrg, repo.Name)
	err = util.UpdateRepositoryRemotes(repo.gitRepo, upstreamRemoteName, []string{expectedUpstreamURL})
	if err != nil {
		return fmt.Errorf("error updating origin URL: %v", err)
	}
	return nil
